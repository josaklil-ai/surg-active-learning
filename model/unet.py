from model.loss import MixedLoss
from model.dice import DiceCoeff
from model.pam import MaskHead

import pytorch_lightning as pl
import torch
from torch import nn
import torch.nn.functional as F
from torchvision import models

def convrelu(in_channels, out_channels, kernel, padding):
    return nn.Sequential(
        nn.Conv2d(in_channels, out_channels, kernel, padding=padding), 
        nn.ReLU(inplace=True),
    )    


class ResNetUNet(pl.LightningModule):
    def __init__(self, args, n_class):
        super().__init__()
        self.args = args
        self.n_class = n_class
        self.calc_loss = MixedLoss()
        self.calc_dice = DiceCoeff(args)
        
        self.base_model = models.resnet18(pretrained=False)
        self.base_layers = list(self.base_model.children())
        
        self.layer0 = nn.Sequential(*self.base_layers[:3])
        self.layer0_1x1 = convrelu(64, 64, 1, 0)
        self.layer1 = nn.Sequential(*self.base_layers[3:5])
        self.layer1_1x1 = convrelu(64, 64, 1, 0)
        self.layer2 = self.base_layers[5]
        self.layer2_1x1 = convrelu(128, 128, 1, 0)
        self.layer3 = self.base_layers[6]
        self.layer3_1x1 = convrelu(256, 256, 1, 0)
        self.layer4 = self.base_layers[7]
        self.layer4_1x1 = convrelu(512, 512, 1, 0)
        
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        
        self.conv_up3 = convrelu(256 + 512, 512, 3, 1)
        self.conv_up2 = convrelu(128 + 512, 256, 3, 1)
        self.conv_up1 = convrelu(64 + 256, 256, 3, 1)
        self.conv_up0 = convrelu(64 + 256, 128, 3, 1)
        
        self.conv_original_size0 = convrelu(3, 64, 3, 1)
        self.conv_original_size1 = convrelu(64, 64, 3, 1)
        self.conv_original_size2 = convrelu(64 + 128, 64, 3, 1)
        
        self.conv_last = nn.Conv2d(64, self.n_class, 1)
        
        # Use dropout layers for BALD acquisition strategy
        p = 0.5 if self.args.query == 10 else 0.0
        
        self.dropout = nn.Dropout(p)

    def forward(self, input):
        x_original = self.conv_original_size0(input)
        x_original = self.conv_original_size1(x_original)
        
        layer0 = self.layer0(input)
        layer1 = self.layer1(layer0)
        layer1 = self.dropout(layer1)
        layer2 = self.layer2(layer1)
        layer2 = self.dropout(layer2)
        layer3 = self.layer3(layer2)
        layer3 = self.dropout(layer3)
        layer4 = self.layer4(layer3)
        
        layer4 = self.dropout(layer4)
        layer4 = self.layer4_1x1(layer4)
        x = self.upsample(layer4)
        layer3 = self.layer3_1x1(layer3)
        x = torch.cat([x, layer3], dim=1)
        x = self.conv_up3(x)
        
        x = self.dropout(x)
        x = self.upsample(x)
        layer2 = self.layer2_1x1(layer2)
        x = torch.cat([x, layer2], dim=1)
        x = self.conv_up2(x)
        
        x = self.upsample(x)
        layer1 = self.layer1_1x1(layer1)
        x = torch.cat([x, layer1], dim=1)
        x = self.conv_up1(x)
        
        x = self.upsample(x)
        layer0 = self.layer0_1x1(layer0)
        x = torch.cat([x, layer0], dim=1)
        x = self.conv_up0(x)
        
        x = self.upsample(x)
        x = torch.cat([x, x_original], dim=1)
        x = self.conv_original_size2(x)
        
        emb = F.avg_pool2d(x, (32, 32))
        out = self.conv_last(x)
        
        mask = None
        mask_head = MaskHead(self.n_class, True)
        if self.args.query == 6:
            mask, _ = mask_head(out)
        
            # out = F.interpolate(out, size=input.size()[2:], mode='bilinear', align_corners=True)
            mask = F.interpolate(mask, size=input.size()[2:], mode='bilinear', align_corners=True)
            mask = torch.sigmoid(mask)
        
        return out, emb, mask
    
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, self.parameters()), lr=self.args.lr)
        return {'optimizer': optimizer}
    
    def training_step(self, batch, batch_idx):
        x, y = batch
        y_hat, _, _ = self(x)
        loss = self.calc_loss(y_hat, y)
        self.log('train_loss', loss)
        return { 'loss': loss }
    
    def validation_step(self, batch, batch_idx):
        x, y = batch
        y_hat, _, _ = self(x)
        loss = self.calc_loss(y_hat, y)
        self.log('val_loss', loss)
        class_dsc = self.calc_dice(y_hat, y)
        return (y_hat, y, class_dsc)
    
    def validation_epoch_end(self, outs):
        class_dsc = torch.vstack([out[2] for out in outs])
        self.log('val_dice', class_dsc.mean())
        for i in range(class_dsc.shape[1]):
            self.log(f'vdsc-c{i}', class_dsc[:, i].mean())
            
    def test_step(self, batch, batch_idx):
        x, y = batch
        y_hat, _, _ = self(x)
        loss = self.calc_loss(y_hat, y)
        self.log('test_loss', loss)
        class_dsc = self.calc_dice(y_hat, y)
        return (y_hat, y, class_dsc)
    
    def test_epoch_end(self, outs):
        class_dsc = torch.vstack([out[2] for out in outs])
        self.log('test_dice', class_dsc.mean())
        for i in range(class_dsc.shape[1]):
            self.log(f'tdsc-c{i}', class_dsc[:, i].mean())

            